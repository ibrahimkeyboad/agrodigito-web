...
const HowItWorksSection = () => {
  return (
    <section
      ...
      <div className='absolute right-0 top-0 w-2/3 h-2/3 bg-agrilink-light/50 rounded-bl-[100%] -z-10'></div>
      ...
      <div className='container mx-auto px-6 md:px-10'>
        ...
        <div className='relative'>
          {/* Connector Line */}
          <div className='hidden md:block absolute top-1/2 left-0 right-0 h-1 bg-agrilink-secondary/30 -z-10 transform -translate-y-1/2'></div>

          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16'>
            {steps.map((step, index) => (
              <div key={index} className='flex flex-col items-center group'>
                <div className='mb-8 flex justify-center relative'>
                  <div className='flex items-center justify-center w-24 h-24 rounded-2xl bg-gradient-to-br from-agrilink-primary to-agrilink-dark shadow-lg group-hover:shadow-xl transition-all duration-300 transform group-hover:scale-105'>
                    {step.icon}
                  </div>
                  <div className='absolute -top-3 -right-3 bg-agrilink-accent w-8 h-8 rounded-full flex items-center justify-center text-agrilink-dark font-bold text-sm shadow-md'>
                    {step.number.split('')[1]}
                  </div>
                </div>
                <div className='text-center'>
                  <h3 className='text-xl font-semibold mb-3 text-gray-800 group-hover:text-agrilink-primary transition-colors'>
                    {step.title}
                  </h3>
